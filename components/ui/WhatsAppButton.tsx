...
/** Réservé à la page /commander : produit obligatoire. */
...
function WhatsAppSetupHint() {
  if (process.env.NODE_ENV === "production") return null;
  return (
    <p className="max-w-sm text-xs leading-relaxed text-muted">
      WhatsApp inactif : ajoutez{" "}
      <code className="text-ink">NEXT_PUBLIC_WHATSAPP_NUMBER=33612345678</code>{" "}
      dans <code className="text-ink">.env.local</code>, puis redémarrez{" "}
      ...
      <div className={className}>
        ...
        {process.env.NODE_ENV === "production" ? (
          <p className="text-sm text-muted">
            Utilisez le formulaire ci-dessous pour cette commande.
          </p>
        ) : null}
      </div>
    );
